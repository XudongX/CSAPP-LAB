...
void Sigaddset(sigset_t *mask, int sign) {
  if (sigaddset(mask, sign) < 0) unix_error("Sigaddset error");
  return;
}
void Sigprocmask(int how, sigset_t *mask, sigset_t *oldmask) {
  if (sigprocmask(how, mask, oldmask) < 0) unix_error("Sigprocmask error");
  return;
}
pid_t Fork(void) {
  pid_t pid;
  ...
  if (!builtin_cmd(argv)) { /*not a build in cmd*/
    ...
    if ((pid = Fork()) == 0) {
      Sigprocmask(SIG_UNBLOCK, &mask,
                  NULL); /*unblock the SIGCHLD signal in child*/
    ...
    Sigprocmask(SIG_UNBLOCK, &mask,
                NULL); /*unblock the SIGCHLD signal in parent*/

    bg ? printf("[%d] (%d) %s", pid2jid(pid), pid, cmdline)
