...
int mm_init(void) {
  ...
  PUT(heap_listp + (1 * WSIZE), 0);               // 序言块header
  PUT(heap_listp + (2 * WSIZE), 0);               // 序言块footer
  PUT(heap_listp + (3 * WSIZE), PACK(DSIZE, 1));  // 空闲链表prev指针
  PUT(heap_listp + (4 * WSIZE), PACK(DSIZE, 1));  // 空闲链表next指针
...
static void printblock(void *bp) {
  ...
  if (!halloc || bp == heap_listp) {
    ...
    printf(">> %p: header: [%d:%c] next: %lx prev: %lx footer: [%d:%c] <<\n",
           bp, (int)hsize, (halloc ? 'a' : 'f'), next, prev, (int)fsize,
           (falloc ? 'a' : 'f'));
  } else {
