...
void simulate(cache_line ***cache, trace_line *t_line, args_list *args,
  ...
  if (!hit_flag) {  // 没有hit则尝试放入cache
    ...
    if (eviction_flag) {
      ...
      (info->num_evictions)++;
      unsigned int max_line_time = 0;
      int max_line_index = -1;
      for (int i = 0; i < args->E; i++) {
        if ((set[i]->time_stamp) > (max_line_time)) {
          max_line_time = set[i]->time_stamp;
          max_line_index = i;
        }
      }
      set[max_line_index]->valid = 1;
      set[max_line_index]->time_stamp = 0;
      set[max_line_index]->tag = tag;
    }
  }
